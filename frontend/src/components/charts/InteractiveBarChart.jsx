    ...
    <div>
      ...
      <ChartContainer height={height}>
        <BarChart data={data} layout={isVertical ? 'vertical' : 'horizontal'}
          ...
          <Bar dataKey={dataKey} radius={isVertical ? [0, 8, 8, 0] : [8, 8, 0, 0]}
            barSize={22} animationDuration={1000}
            onMouseEnter={(_, i) => setActiveBar(i)} onMouseLeave={() => setActiveBar(null)}>
            {data.map((d, i) => {
              const c = d[colorKey] || defaultColor;
              const active = activeBar === i;
              return <Cell key={i} fill={c} fillOpacity={active ? 1 : 0.72}
                style={{ filter: active ? `drop-shadow(0 0 8px ${c})` : 'none', transition: 'all 0.2s ease' }} />;
            })}
          </Bar>
        </BarChart>
      </ChartContainer>
