...
import { Key, Clock, CheckCircle, XCircle, Send, Shield, FileText, AlertTriangle } from 'lucide-react';
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-3">
          <Key className="w-7 h-7 text-sg-red" /> My Access Requests
        </h1>
        <p className="text-slate-400 text-sm mt-1">Request temporary access to platforms and roles — reviewed by security admin</p>
      </div>

      <div className="grid grid-cols-4 gap-3">
        {[
          { label: 'Total', value: stats.total, color: 'text-white', icon: FileText },
          { label: 'Pending', value: stats.pending, color: 'text-yellow-400', icon: Clock },
          { label: 'Approved', value: stats.approved, color: 'text-green-400', icon: CheckCircle },
      ...
      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowForm(!showForm)}
        className="px-6 py-3 rounded-xl bg-gradient-to-r from-red-600 to-red-500 text-white font-semibold text-sm hover:opacity-90 transition-opacity flex items-center gap-2">
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
            <GlassCard hover={false} className="border-red-500/20">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="text-[11px] text-slate-500 uppercase tracking-wider block mb-1">Platform</label>
                ...
                <div>
                  <label className="text-[11px] text-slate-500 uppercase tracking-wider block mb-1">Requested Role</label>
                ...
                <div>
                  <label className="text-[11px] text-slate-500 uppercase tracking-wider block mb-1">Duration</label>
                ...
                <div>
                  <label className="text-[11px] text-slate-500 uppercase tracking-wider block mb-1">Business Justification</label>
                  <input value={form.justification} onChange={e => setForm(f => ({ ...f, justification: e.target.value }))}
                    placeholder="Reason for access..."
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm text-white placeholder-slate-500 outline-none focus:border-red-500/50" />
                </div>
              </div>
      ...
      <GlassCard hover={false} className="border-red-500/10">
        <h3 className="text-sm font-semibold text-slate-300 mb-4 flex items-center gap-2">
          <FileText size={14} className="text-red-400" /> My Requests
        </h3>
        {requests.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-12">
            <Key size={40} className="text-slate-700" />
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-sm font-semibold text-white">{req.role}</span>
                        <span className="text-[10px] text-slate-500">on {PLATFORM_LABELS[req.platform]}</span>
                      ...
                      <p className="text-xs text-slate-400 mt-1">Justification: {req.justification}</p>
                    </div>
                  </div>
                </motion.div>
              );
